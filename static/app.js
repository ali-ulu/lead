...
function showTab(tab){
  ...
  if(tab==="message"){
    n.innerHTML='<div class="tab-panel"><div class="lang-row">'+["en","tr","ur","sd"].map(x=>'<button class="lang-btn" data-lang="'+x+'">'+x.toUpperCase()+'</button>').join("")+'</div><div id="message" class="message-box">Choose language</div><div class="message-actions"><button id="copyMessage" class="secondary-btn" disabled>'+esc(tr("copy"))+'</button></div>'+
      (metaConnections.length?'<div class="audit-card" style="margin-top:14px"><div class="kv"><span>Provider</span><select id="metaProvider"><option value="instagram">Instagram</option><option value="facebook">Facebook</option></select><span>'+esc(tr("recipient"))+'</span><input id="metaRecipient" placeholder="Meta recipient / conversation id"></div><button id="sendMetaBtn" class="primary-btn" style="margin-top:10px">'+esc(tr("send"))+'</button></div>':"")+'</div>';
    ...
    if($("sendMetaBtn"))$("sendMetaBtn").addEventListener("click",()=>sendMeta(lead.id));
  }
  if(tab==="crm"){
...
async function init(){
  try{
    ...
    if(metaConnections.length)$("metaConnect").textContent=tr("meta")+" ("+metaConnections.length+")";
  }catch(e){toast(e.message,true);}
